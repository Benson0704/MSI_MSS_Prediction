'''
This script trains MSS/MSI on 3 cases:
    1.low+middle
    2.low
    3.middle
'''
...
for i in fileA0:  # mss=1 msi=0
    f = open(i, 'r')
    for line in f.readlines():
        dataA0.append(line[:-1])
    f.close()
for i in fileA1:
    f = open(i, 'r')
    for line in f.readlines():
        dataA1.append(line[:-1])
    f.close()
random.shuffle(data0)
random.shuffle(data1)
random.shuffle(dataA0)
random.shuffle(dataA1)
trainSet = []
validationSet = []
testSet = []
...
for i in range(len(dataA1)):  # mss=1 msi=0
    if 170 <= i % 200 < 195:
        validationSet.append((dataA1[i], 1))
    if 195 <= i % 200 < 200:
        testSet.append((dataA1[i], 1))
for i in range(len(dataA0)):  # mss=1 msi=0
    if 170 <= i % 200 < 195:
        validationSet.append((dataA0[i], 0))
    if 195 <= i % 200 < 200:
        testSet.append((dataA0[i], 0))
