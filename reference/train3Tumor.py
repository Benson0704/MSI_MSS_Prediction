'''
This script trains Other/low/middle
'''
import scipy.stats
import torch
import torchvision
...
validationSet = []
testSet = []
for i in range(len(data1)):  # middle=2 low=1 other=0
    if 0 <= i % 200 < 170:
...
for i in range(len(data2)):
    if 0 <= i % 200 < 170:
        trainSet.append((data2[i], 2))
    if 170 <= i % 200 < 195:
        validationSet.append((data2[i], 2))
    if 195 <= i % 200 < 200:
        testSet.append((data2[i], 2))
