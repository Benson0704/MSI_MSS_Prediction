...
f.close()
Write_log('test:{}, testMSS:{}, testMSI:{}'.format(len(
    testFilesMSI)+len(testFilesMSS), len(testFilesMSS), len(testFilesMSI)))
Write_log('train:{}, trainMSS:{}, trainMSI:{}'.format(len(
    trainFilesMSI) + len(trainFilesMSS), len(trainFilesMSS), len(trainFilesMSI)))
...
testSet = []
'''
random.shuffle(trainFilesMSI)
random.shuffle(trainFilesMSS)
'''
for i in range(len(trainFilesMSS)):  # mss=1 msi=0
    if 0 <= i % 200 < 170:
        trainSet.append((trainFilesMSS[i], 1))
    if 170 <= i % 200 < 195:
        validationSet.append((trainFilesMSS[i], 1))
    if 195 <= i % 200 < 200:
        testSet.append((trainFilesMSS[i], 1))
for i in range(len(trainFilesMSI)):  # mss=1 msi=0
    if 0 <= i % 200 < 170:
        trainSet.append((trainFilesMSI[i], 0))
    if 170 <= i % 200 < 195:
        validationSet.append((trainFilesMSI[i], 0))
    if 195 <= i % 200 < 200:
        testSet.append((trainFilesMSI[i], 0))
...
train_dataloader = torch.utils.data.dataloader.DataLoader(
    train_dataset, batch_size=hyper_parameters['BS'], num_workers=hyper_parameters['NW'], shuffle=True)
test_dataset = NewDataset(testSet, torchvision.transforms.Compose([
    torchvision.transforms.ToTensor()]))
test_dataloader = torch.utils.data.dataloader.DataLoader(
    test_dataset, batch_size=hyper_parameters['BS'], num_workers=hyper_parameters['NW'])
...
validation_dataloader = torch.utils.data.dataloader.DataLoader(
    validation_dataset, batch_size=hyper_parameters['BS'], num_workers=hyper_parameters['NW'])
train()
test()
Write_log("start saving!")
torch.save(net, "models/"+FILE_NAME+'.models')
