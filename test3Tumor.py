'''
This script test other-low-middle
'''
import scipy.stats
import torch
import torchvision
...
Write_log('Other: {}, Low: {}, Middle: {}'.format(
    len(data0), len(data1), len(data2)))
L = []
...
for i in range(len(result)):
    ...
    if result[i] == 2:
        ...
        if L[i] == 2:
            real2to2 += 1
Write_log('real0to0: {}, real0to1: {}, real0to2: {}'.format(
    real0to0, real0to1, real0to2))
Write_log('real1to0: {}, real1to1: {}, real1to2: {}'.format(
    real1to0, real1to1, real1to2))
Write_log('real2to0: {}, real2to1: {}, real2to2: {}'.format(
...
Write_log('Low pricision: {}, recall: {}, F1: {}'.format(
    P, R, 2 * P * R / (P + R)))
P = real2to2 / (real0to2 + real1to2 + real2to2)
R = real2to2 / (real2to0 + real2to1 + real2to2)
Write_log('Middle pricision: {}, recall: {}, F1: {}'.format(
    P, R, 2*P*R / (P + R)))
...
Write_log('Tumor(Low+Middle) pricision: {}, recall: {}, F1: {}'.format(
    P, R, 2*P*R / (P + R)))
PLfinal = []
Pfinal = []
for i in range(len(result)):
